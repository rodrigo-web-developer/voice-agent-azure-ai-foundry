...
class AudioProcessor:
    ...
    def pause_capture(self) -> None:
        """Temporarily stop sending microphone audio to the service."""
        self.capture_enabled = False
        logger.debug("Microphone capture paused")

    def resume_capture(self) -> None:
        """Resume sending microphone audio to the service."""
        self.capture_enabled = True
...
class BasicVoiceAssistant:
    ...
    def __init__(
        ...
        self.mute_mic_while_speaking = (
            os.environ.get("AZURE_VOICELIVE_MUTE_MIC_WHILE_SPEAKING", "true").strip().lower() == "true"
        )
        self._mic_muted_for_tts = False
        self.auto_language_voice = (
            os.environ.get("AZURE_VOICELIVE_AUTO_LANGUAGE_VOICE", "true").strip().lower() == "true"
        )
        self.auto_language_allowed = {"en", "pt"}
        self.default_english_voice = os.environ.get("AZURE_VOICELIVE_VOICE_EN", "en-US-Ava:DragonHDLatestNeural")

        self.language_voice_map = {
            "en": self.default_english_voice,
            "pt": os.environ.get("AZURE_VOICELIVE_VOICE_PT", "pt-BR-FranciscaNeural"),
        }
    ...
    async def _setup_session(self) -> None:
        ...
        # Interim responses can sound like duplicate answers in voice mode.
        # Keep them disabled by default and allow explicit opt-in via env.
        enable_interim_responses = (
            os.environ.get("AZURE_VOICELIVE_ENABLE_INTERIM_RESPONSES", "false").strip().lower() == "true"
        )

        interim_response_config: Optional[LlmInterimResponseConfig] = None
        if enable_interim_responses:
            interim_response_config = LlmInterimResponseConfig(
                ...
                instructions=(
                    ...
                    "Do not include in all responses! Do not say you don't have real-time access to information "
                    "when calling tools!"
                ),
            )
        ...
        session_config = RequestSession(
            ...
            # Always set voice here so local language/voice preferences override stale server defaults.
            voice=AzureStandardVoice(name=self.current_voice),
    ...
    async def _maybe_update_voice_for_transcript(self, transcript: str) -> None:
        """Switch voice to match detected language for more natural accent."""
        if not self.auto_language_voice:
            return
        ...
        if language not in self.auto_language_allowed:
            logger.debug("Ignoring auto language detection for unsupported language: %s", language)
            return

        target_voice = self.language_voice_map.get(language)
        if not target_voice or target_voice == self.current_voice:
            return
        ...
        try:
            await conn.session.update(session=RequestSession(voice=AzureStandardVoice(name=target_voice)))
            logger.info("Auto-switched voice from %s to %s for language %s", self.current_voice, target_voice, language)
            self.current_voice = target_voice
            print(f"🔊 Voice switched for language {language}: {target_voice}")
        except Exception:
            logger.exception("Failed to auto-switch voice for language %s", language)
    ...
    async def _handle_event(self, event: Any) -> None:
        ...
        if event.type == ServerEventType.SESSION_UPDATED:
            ...
            await self._maybe_update_voice_for_transcript(transcript)
            ...
            if self._mic_muted_for_tts:
                logger.debug("Ignoring speech-start event while mic is muted for assistant speech")
                return
            ...
            self._response_api_done = False
            if self.mute_mic_while_speaking and not self._mic_muted_for_tts:
                ap.pause_capture()
                self._mic_muted_for_tts = True
...
def main() -> None:
    ...
    agent_authentication_identity_client_id = os.environ.get("AZURE_VOICELIVE_AUTH_IDENTITY_CLIENT_ID")
    ...
    print(f"AZURE_VOICELIVE_AUTH_IDENTITY_CLIENT_ID: {agent_authentication_identity_client_id}")

    if not endpoint or not agent_name or not project_name:
        sys.exit("Set AZURE_VOICELIVE_ENDPOINT, AZURE_VOICELIVE_AGENT_ID, and AZURE_VOICELIVE_PROJECT_NAME in your .env file.")
